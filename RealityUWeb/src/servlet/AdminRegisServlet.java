...
/**
 * Servlet implementation class AdminRegisServlet Admin Registration process,
 * check for existing userid/password Save new userid/password in dbase
 */
	...
	protected void processRequest(HttpServletRequest request,
		...
		try {
			...
			if (request.getParameter("clear") != null
				...
				// the regis.jsp page doesn't fill in form values from the other
				// obj
				...
				RequestDispatcher rd = getServletContext()
						.getRequestDispatcher("/regis.jsp");
				...
				// Create AdministratorDAO & Administrator Objs and Validate
				// Registration
				...
				// Returns List of Administrators matching search criteria (even
				// if only 1)
				lstAdminUserName = adao1.search("username", username);
				System.out
						.println("Acquired List of Administrator objs. Num matches User Name: "
								+ lstAdminUserName.size());
				...
				System.out
						.println("Acquired List of Administrator objs. Num matches Password: "
								+ lstAdminPW.size());

				if (lstAdminUserName.size() == 0 && lstAdminPW.size() == 0) {
					// Valid Registration - No username match and no password
					// match
					...
					// Create Administrator obj to hold user-entered form data
					// values
					// Dummy id, don't add to dbase. Key: id 1=dupe userid,
					// 2=dupe pw, 3=dupe both
					int keyID = 0;
					...
					Administrator adm2 = new Administrator(keyID, username,
					...
					rd.forward(request, response);
					System.out
							.println("ERROR! Registration Invalid. Try again.");
				} // end if
			} // end clear button if
		} catch (Exception e) {
			// Handle Errors
