...
public class SubmitSurveyServlet extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request,
		...
		String pgID = ""; // To hold pageID to determine if Admin (reg. vs.
							// processed) or Student
		...
		if (ses.getAttribute("surveyviewGrp") != null) {
			...
			// Read in Group obj from Session for ADMIN PROCESSED
			// (surveyprocessed.jsp)
		...
		try {
			...
			if (request.getParameter("clear") != null
				...
				gpa = (request.getParameter("gpa") == null) ? "0.0" : request
						.getParameter("gpa");

				gender = request.getParameter("gender");
				education = request.getParameter("education");
				// Category/Industry won't exist for Processed Admin Surveys
				category = (request.getParameter("industry") == null) ? ""
						: request.getParameter("industry");
				// prefJob won't exist for Processed Admin Surveys
				prefJob = (request.getParameter("occupation") == null) ? ""
						: request.getParameter("occupation");

				// Job won't exist for Student Surveys
				job = (request.getParameter("job") == null) ? "" : request
						.getParameter("job");
				married = request.getParameter("married");
				// Spouse won't exist for Student Surveys
				spouse = (request.getParameter("spouse") == null) ? ""
						: request.getParameter("spouse");
				children = request.getParameter("children");
				numChild = (request.getParameter("numchild") == null) ? "0"
						: request.getParameter("numchild");

				cCards = request.getParameter("creditcard");
				cCardUses = (request.getParameter("cc_use") == null) ? ""
						: request.getParameter("cc_use");

				// groceries, clothing, home & vehicle won't exist for Processed
				// Admin Surveys
				groceries = (request.getParameter("groceries") == null) ? ""
						: request.getParameter("groceries");
				clothing = (request.getParameter("clothing") == null) ? ""
						: request.getParameter("clothing");
				home = (request.getParameter("home") == null) ? "" : request
						.getParameter("home");
				vehicle = (request.getParameter("vehicle") == null) ? ""
						: request.getParameter("vehicle");

				// childSupport & creditScore won't exist for Student Surveys
				childSupport = (request.getParameter("childSupport") == null) ? ""
						: request.getParameter("childSupport");
				creditScore = (request.getParameter("creditScore") == null) ? ""
						: request.getParameter("creditScore");

				// save & entertainment won't exist for Processed Admin Surveys
				save = (request.getParameter("save") == null) ? "" : request
						.getParameter("save");
				entertainment = (request.getParameter("entertain") == null) ? ""
						: request.getParameter("entertain");
				System.out.println("Read in Form Parameters.");

				// Get value of Group ID: if ADMIN use 'groupID' from form, if
				// STUDENT use grp object
				int groupID = (request.getParameter("groupID") == null) ? grp
						.getId() : Integer.parseInt(request
						.getParameter("groupID"));

				// If "submit" button clicked, not "editSurvey" button -->
				// editSurvey button is null
				// Create New Survey
				if (request.getParameter("editSurvey") == null) {
					System.out.println("CREATE NEW SURVEY.");

					// Submit survey
					// Create holder Survey obj to hold form data values. With
					// id=0
					sd1.insert(fName, lName, dob, Double.parseDouble(gpa),
							gender, groupID, education, prefJob, "", married,
							0, children, Integer.parseInt(numChild), cCards,
							cCardUses, groceries, clothing, home, vehicle, 0.0,
							0.0, save, entertainment);
					System.out.println("Inserted new Survey into dbase.");

					// Lookup obj values from dbase to get id, lookup by dob,
					// and compare to match on dob & last name
					// Assign matching Survey to s1 object
					boolean found = false;
					lstSurvey = sd1.search("dob", dob);
					for (int i = 0; i < lstSurvey.size(); i++) {
						found = lstSurvey.get(i).validateLogin(dob, lName);
						if (found) {
							s1 = lstSurvey.get(i);
						}
					} // end for
					...
					surveyMsg = "Thank you, " + s1.getFname()
							+ ". Your survey has been submitted successfully.";
					System.out.println("New survey submitted.");

				} else { // "editSurvey" button clicked
					// Edit Existing Survey: FOR STUDENT (survey.jsp) OR ADMIN
					// REG (surveyview.jsp) OR ADMIN PROCESSED
					// (surveyprocessed.jsp)
					System.out.println("EDIT EXISTING SURVEY.");

					// Read in Survey ID
					surveyID = request.getParameter("surveyID");
					// Get values from current Survey object that are not
					// changed on the form (Reg. vs. Processed)
					// and use those values so don't overwrite data needed with
					// blanks
					Survey s = sd1.find(Integer.parseInt(surveyID));
					System.out.println("EDIT EXISTING SURVEY Entertainment: "
							+ s.getEntertainment() + ", Job: " + s.getJob());

					if (pgID.equals("adminSurveyEditProcessed")) {
						// PROCESSED SURVEY
						// Create new Survey with same surveyID but new/edited
						// values
						s1 = new Survey(Integer.parseInt(surveyID), fName,
								lName, dob, Double.parseDouble(gpa), gender,
								groupID, education, s.getPrefJob(), job,
								married, Integer.parseInt(spouse), children,
								Integer.parseInt(numChild), s.getCCards(),
								s.getCCardUses(), s.getGroceries(),
								s.getClothing(), s.getHome(), s.getVehicle(),
								Double.parseDouble(childSupport),
								Double.parseDouble(creditScore), s.getSave(),
								s.getEntertainment());
					} else {
						...
						s1 = new Survey(Integer.parseInt(surveyID), fName,
								lName, dob, Double.parseDouble(gpa), gender,
								groupID, education, prefJob, s.getJob(),
								married, s.getSpouse(), children,
								Integer.parseInt(numChild), cCards, cCardUses,
								groceries, clothing, home, vehicle,
								s.getChildSupport(), s.getCreditScore(), save,
								entertainment);
					} // end if
					...
					if (pgID.equals("adminSurveyEdit")
							|| pgID.equals("adminSurveyEditProcessed")) {
						// Admin message
						surveyMsg = "Thank you. The Survey has been edited successfully.";
					} else {
						// Student message
						surveyMsg = "Thank you, " + s1.getFname()
								+ ". Your Survey has been edited successfully.";
					} // end if
			...
			// Process differently for Admin Edit of Survey vs. Student Survey
			// Submission/Edit
			if (pgID.equals("adminSurveyEdit")
					|| pgID.equals("adminSurveyEditProcessed")) {
				...
				if (request.getParameter("delete") != null) {
					...
					RequestDispatcher rd = getServletContext()
							.getRequestDispatcher("/surveydeleted.jsp");
					...
					if (pgID.equals("adminSurveyEdit")) {
						...
						RequestDispatcher rd = getServletContext()
								.getRequestDispatcher("/surveyview.jsp");
						rd.forward(request, response);
					} else {
						// PROCESSED ADMIN SURVEY: Put Objs in Session
						// (different names)
						HttpSession ses1 = request.getSession();
						...
						RequestDispatcher rd = getServletContext()
								.getRequestDispatcher("/surveyprocessed.jsp");
						rd.forward(request, response);
					}
				} // end if
			} else {
