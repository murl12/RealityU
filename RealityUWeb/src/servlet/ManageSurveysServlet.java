	...
	protected void processRequest(HttpServletRequest request,
		...
		try {
			...
			if (request.getParameter("delete") != null) {
				...
				// No message, will update list of student surveys for group
				...
				surveyviewGrp = gd.find(viewSurvey.getGroupID()); // Get Group
																	// ID from
																	// deleted
																	// Survey
				...
				lstSurveys = sd.search("groupID",
						("" + viewSurvey.getGroupID())); // Use String groupID
															// value for
															// 'search'
															// parameter which
															// is String
				...
				ses1.setAttribute("openGrp", surveyviewGrp); // name Group obj
																// differently
				...
				RequestDispatcher rd = getServletContext()
						.getRequestDispatcher("/opengroup.jsp");
				rd.forward(request, response);
				System.out.println("Go to Survey Deleted confirmation page.");

			} else if (request.getParameter("view") != null
					|| request.getParameter("processed") != null) {
				// VIEW INDIVIDUAL STUDENT SURVEY BY surveyID (REGULAR OR
				// PROCESSED SURVEYS)
				surveyID = request.getParameter("surveyID");
				...
				if (request.getParameter("view") != null) {
					...
					// Get Category from Occupation object, should only be 1 obj
					// in list for this job
					...
					rd.forward(request, response);
					System.out
							.println("Go to View PROCESSED Survey page in new window.");
