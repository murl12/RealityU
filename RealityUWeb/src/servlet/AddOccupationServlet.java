...
public class AddOccupationServlet extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request,
		...
		String occupsMsg = null; // msg for occupations page --> need to clear
									// out when Occupations table refreshed
		...
		try {
			...
			if (request.getParameter("clear") != null
				...
				occupID = (request.getParameter("occpsID") == null) ? "0"
						: request.getParameter("occpsID");
				...
				if (request.getParameter("editOccp") != null) {
					...
					// Create Occupation
					editOccp = new Occupation(Integer.parseInt(occupID), name,
							type, industry, category,
							Double.parseDouble(annGrossSal),
							Double.parseDouble(monGrossSal),
							Double.parseDouble(marAnnualTax),
							Double.parseDouble(marMonthlyTax),
							Double.parseDouble(marAfterTax),
							Double.parseDouble(sinAnnualTax),
							Double.parseDouble(sinMonthlyTax),
							Double.parseDouble(sinAfterTax),
							Integer.parseInt(gpaCategory),
					...
					ocd.insert(name, type, industry, category,
							Double.parseDouble(annGrossSal),
							Double.parseDouble(monGrossSal),
							Double.parseDouble(marAnnualTax),
							Double.parseDouble(marMonthlyTax),
					...
					// Create Occupation obj by using values from dbase --> to
					// get id, lookup by name (which should be unique)
					lstOccp = ocd.search("name", name);
					// Create Occupation obj: Occupation List should only have
					// one value, so use index 0
