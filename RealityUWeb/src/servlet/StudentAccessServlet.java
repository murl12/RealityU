...
/**
 * Servlet implementation class StudentAccessServlet Student Access process.
 * Validate Student Access Code given and then check if matches existing student
 * last name & dob. If it matches, the survey has already been completed and
 * they cannot edit -> go to message page. If name/dob doesn't match, they go to
 * Survey page. If Student Access Code is invalid, they go to invalid login
 * page.
 */
...
public class StudentAccessServlet extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request,
		...
		Survey s1 = null; // reset survey session object so doesn't use previous
							// survey info on new form
		String surveyMsg = null; // Reset to null for new form so msg doesn't
									// show up
		...
		try {
			...
			// Create GroupsDAO & Group Objs and search for matching access code
			// in dbase
			...
			lstGrp = gd.search("studentAccessCode", studentAccesscode); // Lookup
																		// by
																		// access
																		// code
			System.out
					.println("Acquired List of Group objs. Num matches for Access Code: "
							+ lstGrp.size());
			...
			if (lstGrp.size() == 1) { // 1=valid access code match found
				...
				System.out
						.println("Acquired List of Survey objs. Num matches DOB: "
								+ lstSurvey.size());

				if (lstSurvey.size() == 0) {
					// Valid Login - No dob match, so student has not completed
					// a Survey

					// Put Group Obj and Strings for Last Name & DOB in Session
					grp = lstGrp.get(0); // First and only obj in list is index
											// 0
					...
					ses1.setAttribute("surveyMsg", surveyMsg); // Reset to null
																// for new form
					System.out.println("Group: " + grp.getName()
							+ ", Last Name: " + lastName + " & DOB: " + DofB
							+ " added to Session.");
					...
					// Found dob match. Search for Last Name match. (Both must
					// match)

					int flag = 0;
					// Loop thru Survey List (could be multiple objs in list if
					// dupe bdays)
					for (int i = 0; i < lstSurvey.size(); i++) {
						surv = lstSurvey.get(i);
						System.out.println("Extracted Survey obj " + i
								+ " from List.");
						surv.display();

						if (surv.validateLogin(DofB, lastName)) {
							// Combined match on name/dob: Student has already
							// completed a survey
							flag = 1;
					...
					if (flag == 1) {
						...
						RequestDispatcher rd = getServletContext()
								.getRequestDispatcher("/studentNoAccess.jsp");
						rd.forward(request, response);
						System.out
								.println("INVALID: Student has already completed survey.");

					} else {
						// No match on Last Name so Student can complete new
						// Survey

						// Put Group Obj and Strings for Last Name & DOB in
						// Session
						grp = lstGrp.get(0); // First and only obj in list is
												// index 0
						System.out.println("Extracted Group obj from List.");
						HttpSession ses1 = request.getSession();
						ses1.setAttribute("grp", grp);
						ses1.setAttribute("lastName", lastName);
						ses1.setAttribute("DofB", DofB);
						ses1.setAttribute("s1", s1); // Reset to null for new
														// form
						ses1.setAttribute("surveyMsg", surveyMsg); // Reset to
																	// null for
																	// new form
						System.out.println("Group: " + grp.getName()
								+ ", Last Name: " + lastName + " & DOB: "
								+ DofB + " added to Session.");
						System.out.println("surveyMsg: " + surveyMsg);
						grp.display();
