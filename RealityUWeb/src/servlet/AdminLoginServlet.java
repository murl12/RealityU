...
/**
 * Servlet implementation class AdminLoginServlet Admin Login process. Validate
 * userid/password given and forward to valid login or invalid login.
 */
...
public class AdminLoginServlet extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request,
		...
		try {
			...
			// Returns List of Administrators matching search criteria (even if
			// only 1)
			lstAdmin = adao1.search("username", userID); // Lookup by username
			System.out
					.println("Acquired List of Administrator objs. Num matches: "
							+ lstAdmin.size());

			// Extract single Administrator obj from List
			// Loop thru Administrator List (should only be 1 obj or none in
			// list)
			for (int i = 0; i < lstAdmin.size(); i++) {
				if (i == 0) {
					adm = lstAdmin.get(i);
					System.out
							.println("Extracted Administrator obj from List.");
			...
			if (adm.validateLogin(userID, pw)) {
				...
				// Check if need regular Login or Master Login (for
				...
				if (request.getParameter("checkMasterLogin") == null) {
					...
					// Both first and last name must equal "Master" to be the
					// Master Login
					if (adm.getFname().equals("Master")
							&& adm.getLname().equals("Master")) {
						// Put Administrator Obj in Session
						HttpSession ses1 = request.getSession();
						ses1.setAttribute("adm", adm);
						System.out.println("Administrator added to Session.");
