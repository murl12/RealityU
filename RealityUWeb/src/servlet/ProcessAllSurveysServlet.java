	...
	protected void processRequest(HttpServletRequest request,
		...
		try {
			...
			if (request.getParameter("submit") != null) {
				...
				System.out
						.println("Acquired List of Survey objs. w/ Group ID: "
								+ groupID + ". Num of Surveys: "
								+ lstSurveys.size());
				...
				if (lstSurveys.size() > 19) {
					...
					// lstSurveys = new
					// ProcessChildrenDivorcedFemales().doProcess(lstSurveys);
					// lstSurveys = new
					// ProcessChildrenDivorcedMales().doProcess(lstSurveys);
					// lstSurveys = new
					// ProcessCustodyChildSupport().doProcess(lstSurveys);

					msg = "Surveys for " + grp.getName()
					...
					msg = "Surveys for "
							+ grp.getName()
							+ " have been successfully processed to assign Occupations, "
				...
				System.out
						.println("Group added to Session for 'opengroup.jsp' & isProcessed = "
								+ isProcessed);
