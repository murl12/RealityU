...
public class OccupationsDAO implements DAO {
	...
	public List<Occupation> search(String column, String search) {
		...
		try {
			...
			if (column != null) {
				...
				if (column.equals("id") || column.equals("annGrossSal")
						|| column.equals("monGrossSal")
						|| column.equals("marAnnualTax")
						|| column.equals("marMonthlyTax")
						|| column.equals("marAfterTax")
						|| column.equals("sinAnnualTax")
						|| column.equals("sinMonthlyTax")
						|| column.equals("sinAfterTax")
						|| column.equals("gpaCategory")
						|| column.equals("loan")) {
	...
	public List<String> findAllCategories() {
		...
		/*
		 * //Display in Console for Testing Purposes Only for (String str :
		 * lstCategories) { System.out.println("Category ArrayList : " + str); }
		 * System.out.println("==========================="); for (String str :
		 * tempSet) { System.out.println("TempSet List : " + str); }
		 */

		return lstCategories;
	}

	// ============== SELECT/FIND - LIST OF ALL OCCUPATIONS IN EACH CATEGORY
	// (STRING LIST OF OCCUPATION NAMES) ===============
	/**
	 * Creates a String List of all Occupation names for each category.
	 * 
	 * @return Returns a List of all Categories, whereby each Category List
	 *         contains an inner List of Occupation names within that category.
	 */
	public List<List<String>> findOccupationNamesPerCategory() {
		...
		for (int i = 0; i < lstCategories.size(); i++) {
			...
			// (Note: Call 'search' method directly to find all Occupations
			// within one category only)
			lstOccupation = search("category", lstCategories.get(i));
			// System.out.println("Category " + i + " : " +
			// lstCategories.get(i));

			// Makes a secondary(nested) anonymous String list for each nested
			// list
			lstOccupsByCats.add(new ArrayList<String>());

			// Loop thru all Occupation objs and add name of Occupation for each
			// to String List
			for (int j = 0; j < lstOccupation.size(); j++) {
				// Add name of Occupation to nested String Occupation Names List
				// inside of overall 'lstOccupsByCats' array
				lstOccupsByCats.get(i).add(lstOccupation.get(j).getName());
				// System.out.println("Occupation " + j + " : " +
				// lstOccupation.get(j).getName());
			} // end for
		} // end for
		 ...
		 * //Get length of each nested ArrayList -->
		 * lstOccupsByCats.get(i).size() for (int j = 0; j <
		 * lstOccupsByCats.get(i).size(); j++) {
		 * System.out.println("Occupation: " + lstOccupsByCats.get(i).get(j)); }
		 * //end for } //end for
		 */
		// Alternative method: Display in Console for Testing Purposes Only
		/*
		 * int c = 0; for (List<String> cats : lstOccupsByCats) {
		 * System.out.println("============" + lstCategories.get(c) +
		 * "==============="); c++; for (String occps : cats) {
	 ...
	 * 
	 * @return Returns a List of all Categories, whereby each Category List
	 *         contains an inner List of Occupation objects within that
	 *         category.
	 */
	public List<List<Occupation>> findOccupationsPerCategory() {
		...
		/*
		 * //Display in Console for Testing Purposes Only for (List<Occupation>
		 * lst : lstOccupsByCats) {
		 * System.out.println("==========================="); for (Occupation oc
		 * : lst) { System.out.println("Category: " + oc.getCategory() +
		 * ", Occupation: " + oc.getName()); } }
		 */

		return lstOccupsByCats;
	}

	// ================= SELECT/FIND - STRING LIST OF ALL TYPES & INDUSTRIES FOR
	// EACH CATEGORY ================
	/**
	 * Creates a String List of all Types and Industries for each Category.
	 * 
	 * @return Returns a String List of all Categories, whereby each Category
	 *         List contains an inner List of the Type & Industry for that
	 *         Category.
	 */
	public List<List<String>> findTypesAndIndustriesPerCategory() {
		...
		for (int i = 0; i < lstCategories.size(); i++) {
			// Search for all Occupations in each Category (only need one
			// Occupation to check Type & Industry)
			// (Note: Call 'search' method directly to find all Occupations
			// within one category only)
			lstOccupation = search("category", lstCategories.get(i));
			// System.out.println("Category " + i + " : " +
			...
			// Check 1st Occupation in list to get Type & Industry for that
			// Category (only need to check one in list)
		...
		// Display in Console for Testing Purposes Only
		/*
		 * for (int i = 0; i < lstOccupsByTypeAndIndustry.size(); i++) { String
		 * itemName = ""; System.out.println("============ Category " + i + ": "
		 * + lstCategories.get(i) + "===============");
		 * System.out.println("Nested Array Values: " +
		 * lstOccupsByTypeAndIndustry.get(i)); //Get length of each nested
		 * ArrayList --> lstOccupsByTypeAndIndustry.get(i).size() for (int j =
		 * 0; j < lstOccupsByTypeAndIndustry.get(i).size(); j++) { itemName = (j
		 * == 0) ? "Type: " : "Industry: "; System.out.println(itemName +
		 * lstOccupsByTypeAndIndustry.get(i).get(j)); } //end for } //end for
		 */
		// Alternative method: Display in Console for Testing Purposes Only
		/*
		 * int c = 0; for (List<String> cats : lstOccupsByTypeAndIndustry) {
		 * System.out.println("============" + lstCategories.get(c) +
		 * "==============="); c++; for (String occps : cats) {
	...
	public int update(Occupation ocp) {
		...
		try {
			...
			String sql = "UPDATE Occupation SET name=?, type=?, industry=?, category=?, "
					...
					+ "sinAnnualTax=?, sinMonthlyTax=?, sinAfterTax=?, gpaCategory=?, loan=? "
					+ "WHERE id=?";
	...
	public boolean createTable() {
		...
		String sql = "CREATE TABLE IF NOT EXISTS " + tableName
				...
				+ "'name' VARCHAR NOT NULL," + "'type' VARCHAR,"
				+ "'industry' VARCHAR," + "'category' VARCHAR,"
				+ "'annGrossSal' DOUBLE," + "'monGrossSal' DOUBLE,"
				+ "'marAnnualTax' DOUBLE," + "'marMonthlyTax' DOUBLE,"
				+ "'marAfterTax' DOUBLE," + "'sinAnnualTax' DOUBLE,"
				+ "'sinMonthlyTax' DOUBLE," + "'sinAfterTax' DOUBLE,"
				+ "'gpaCategory' INTEGER," + "'loan' DOUBLE)";
	...
	public static void main(String[] args) {
		...
		/*
		 * //Test List of all Categories - Get Occupation Names OccupationsDAO
		 * oc2 = new OccupationsDAO(); oc2.findAllCategories(); //Test List of
		 * Lists (Occupation Names String List inside Categories List)
		 * List<List<String>> loc = new ArrayList<List<String>>(); loc =
		 * oc2.findOccupationNamesPerCategory();
		 */

		// Test List of all Categories - Get Type & Industry
		OccupationsDAO oc2 = new OccupationsDAO();
		// Test List of Lists (Type & Industry String List inside Categories
		// List)
