...
public class ProcessOccupations {
	...
	/**
	 * Do processing to assign an Occupation to each Survey based on GPA &
	 * Preferred Occupation. Student Loans are obtained via the Occupation table
	 * (Loan based on assigned Occupation), no calculation needed for loans
	 * 
	 * @param the
	...
	public List<Survey> doProcess(List<Survey> surveysList) {
		...
		for (Survey survey : surveysList) {
			// Use preferred job to look up Occupation name (should only be 1
			// match)
			...
			System.out.println("Assigned to GPA Category: " + surveyGpaCat
					+ ", Actual GPA: " + survey.getGpa());

			// Compare student survey gpa category to Occupation gpaCategory
			if (surveyGpaCat >= gpaCat) { // student gpa is high enough to get
											// their preferred job
			...
			// *********************************************************************************
				...
				// Get the jobs in the same category, send occupation, and
				// gpaCategory
				eligibleJobsList = getJobsInCategory(occupsList.get(0),
						surveyGpaCat);
				System.out.println("Pick from jobs in Category: "
						+ eligibleJobsList.size());

				// If no jobs, get jobs in same industry
				if (eligibleJobsList.size() < 1) {
					eligibleJobsList = getJobsInIndustry(occupsList.get(0),
							surveyGpaCat);
					System.out.println("Pick from jobs in Industry: "
							+ eligibleJobsList.size());
				}
				// If no jobs, get jobs in same type
				if (eligibleJobsList.size() < 1) {
					eligibleJobsList = getJobsInType(occupsList.get(0),
							surveyGpaCat);
					System.out.println("Pick from jobs in Type: "
							+ eligibleJobsList.size());
				}
				// If no jobs, get jobs in same GPA category
				if (eligibleJobsList.size() < 1) {
					eligibleJobsList = getJobsInGPA(surveyGpaCat);
					System.out.println("Pick from jobs in same GPA Category: "
							+ eligibleJobsList.size());
				}
				// If no jobs, get lowest GPA jobs
				if (eligibleJobsList.size() < 1) { // This will get the minimum
													// number of jobs
					eligibleJobsList = getLowestGPAJobs(surveyGpaCat);
					System.out.println("Pick from jobs in lower GPA Category: "
							+ eligibleJobsList.size());
				...
				if (eligibleJobsList.size() > 0) {
					...
					if (eligibleJobsList.size() == 1) {
						...
						sd.update(survey);
						System.out.println("Assign job from only one choice: "
								+ eligibleJobsList.get(0).getName());
					} else {
						// Go through the list and pick a random job
						Random rndJob = new Random();
						int randomArrayNumber = rndJob.nextInt(eligibleJobsList
								.size());
						survey.setJob(eligibleJobsList.get(randomArrayNumber)
								.getName());
						sd.update(survey);
						System.out.println("Assign job randomly: "
								+ eligibleJobsList.get(randomArrayNumber)
										.getName());
					}
	...
	private List<Occupation> getJobsInCategory(Occupation oc, int surveyGpaCat) {
		...
		List<Occupation> preferredJobsList = checkGPA(
				od.search("category", category), surveyGpaCat);
	...
	private List<Occupation> getJobsInIndustry(Occupation oc, int surveyGpaCat) {
		...
		List<Occupation> lstPrefJobs = checkGPA(
				od.search("industry", industry), surveyGpaCat);
	...
	private List<Occupation> getJobsInType(Occupation oc, int surveyGpaCat) {
		...
		List<Occupation> lstPrefJobs = checkGPA(od.search("type", type),
				surveyGpaCat);
	...
	private List<Occupation> getJobsInGPA(int surveyGpaCat) {
		...
		OccupationsDAO od = new OccupationsDAO();
		// All jobs in all industries having same gpa category as student's
		// actual gpa category
		List<Occupation> lstPrefJobs = od.search("gpaCategory", ""
				+ surveyGpaCat); // have to convert to String for 'search'
									// method
	...
	private List<Occupation> getLowestGPAJobs(int surveyGpaCat) {
		...
		OccupationsDAO od = new OccupationsDAO();
		// Sends list of all Occupations in dbase, and returns all those lower
		// than the student's actual gpa category
		List<Occupation> lowestGPAJobsList = checkGPA(od.findAllOccupations(),
				(surveyGpaCat - 1));
	 ...
	 */
	private List<Occupation> checkGPA(List<Occupation> jobsList,
			int surveyGpaCat) {
